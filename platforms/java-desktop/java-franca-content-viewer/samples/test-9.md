---
title: Obsidian Test Document
tags: [test, markdown, obsidian]
aliases: [Test Document]
---

# Heading Level 1

## Heading Level 2

### Heading Level 3

#### Heading Level 4

##### Heading Level 5

###### Heading Level 6

---

This is a regular paragraph with **bold text**, *italic text*, and ~~strikethrough text~~. You can also use ==highlight==.

A second paragraph with a line break inside it<br>and another line after it.

This paragraph ends with two spaces for a soft break.  
And here is the continuation.

---

## Lists

### Ordered List

1. First item
2. Second item
   1. Subitem 2.1
   2. Subitem 2.2
3. Third item

### Unordered List

- List item
- Nested item
  - Level 2
    - Level 3
- Another item

### Task List

- [x] Completed task
- [ ] Incomplete task
- [ ] Another task

---

## Table

| Header 1                        | Header 2 | Header 3   |
|---------------------------------|--------|------------|
| Cell 1, Line1<b>Cell1 Line2 | Cell 2 | Cell 3     |
| Cell 4                          | Cell 5 | Cell 6     |
| Cell 7                          | Cell 8 | Cell 9     |

---
